...
const addDependencies = (options) => (tree, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    Object.keys(pkgJson.peerDependencies).forEach(pkg => {
        if (pkg.includes('web-animations')) {
            ...
            exports.addPackageToPkgJson(tree, pkg, version, PackageTarget.REGULAR);
            logIncludingDependency(context, pkg, version);
            return;
        }
    });
    exports.addPackageToPkgJson(tree, schematicsPackage, pkgJson.igxDevDependencies[schematicsPackage], PackageTarget.DEV);
});
exports.addDependencies = addDependencies;
/** Checks whether a property exists in the angular workspace. */
const propertyExistsInWorkspace = (targetProp, workspace) => {
    const foundProp = exports.getPropertyFromWorkspace(targetProp, workspace);
    ...
    for (const key of workspaceKeys) {
        ...
        if (Array.isArray(workspace[key])) {
            ...
            if (workspace.hasOwnProperty(key)) {
                const newValue = exports.getPropertyFromWorkspace(targetProp, workspace[key], key);
                if (newValue) {
                    return newValue;
                }
            }
...
const addHammerToConfig = (project, tree, config, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const tsPath = exports.getConfigFile(project, 'main', context, config);
...
const includeDependencies = (pkgJson, context, tree) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const pkg of Object.keys(pkgJson.dependencies)) {
        ...
        switch (pkg) {
            case 'hammerjs':
                logIncludingDependency(context, pkg, version);
                exports.addPackageToPkgJson(tree, pkg, version, entry.target);
